...
public class JsonUtil extends ObjectMapper {
	...
	public JsonUtil() {
		...
		final String dataFormat = DatePattern.NORM_DATETIME_PATTERN;
		if (StrUtil.isNotBlank(dataFormat)){
			this.registerModule(new SimpleModule().addSerializer(Date.class, new JsonSerializer<Date>(){
				...
				public void serialize(Date value, JsonGenerator jgen,
						SerializerProvider provider) throws IOException, JsonProcessingException {
					if (value != null){
						jgen.writeString(DateUtil.format(value, dataFormat));
					}
				}
	        }));
		}
...
//        }));
	...
	public <T> T fromJsonString(String jsonString, Class<T> clazz) {
		if (StrUtil.isEmpty(jsonString) || "<CLOB>".equals(jsonString)) {
			return null;
		}
		try {
	...
	public static String toJsonp(String functionName, Object object){
		return JsonUtil.getInstance().toJsonpString(functionName, object);
	}
	...
	public static <T> T fromJson(String jsonString, Class<?> clazz){
		return (T) JsonUtil.getInstance().fromJsonString(jsonString, clazz);
	}
	...
	public static List<Map<String, Object>> fromJsonForMapList(String jsonString){
		...
		}else if (StrUtil.startWith(jsonString, '[')){
	...
	public static void main(String[] args) {
		...
		List<Map<String, Object>> map2 = JsonUtil.fromJson(json, List.class);
		System.out.println(map2);
		Map<String, Object> map3 = JsonUtil.fromJson("{extendS1:{title:'站牌号',"
